...
class CallProcessor:
    ...
    def recv(self, frame: av.AudioFrame) -> av.AudioFrame:
        ...
        if self.tts_queue:
            pcm_out = self.tts_queue.pop(0)
            return av.AudioFrame.from_ndarray(
                np.frombuffer(pcm_out, np.int16).reshape(-1, frame.layout.channels),
                format="s16",
    ...
    def _pipeline_worker(self):
        SAMPLE_RATE = 16000
        CHUNK_SECONDS = 0.5
        MIN_BYTES = int(SAMPLE_RATE * 2 * CHUNK_SECONDS)

        while True:
            time.sleep(CHUNK_SECONDS)
            with self.lock:
                length = len(self.pcm_buffer)
                if length < MIN_BYTES:
                    continue
                pcm_segment = bytes(self.pcm_buffer)
            ...
            try:
                ...
                print(f"[worker] transcript: {transcript!r}")
            except Exception as e:
                print("[worker] Whisper error:", e)
                continue
            ...
            for chunk in chat_stream:
                ...
                if not token:
                    continue
                print(f"[worker] LLM token: {token!r}")

                try:
                    tts_resp = eleven.text_to_speech.stream(
                        voice=elevenlabs_voice_id,
                        text=token,
                        optimize_streaming_latency=3,
                    )
                except Exception as e:
                    print("[worker] ElevenLabs TTS error:", e)
                    continue

                for mp3_chunk in tts_resp:
                    print(f"[worker] received {len(mp3_chunk)} bytes of MP3")
                    audio_seg = AudioSegment.from_file(BytesIO(mp3_chunk), format="mp3")
                    pcm_chunk = (
                        audio_seg
                    ...
                    with self.lock:
                        self.tts_queue.append(pcm_chunk)
                    print("[worker] enqueued PCM chunk")
...
webrtc_ctx = webrtc_streamer(
    ...
    audio_processor_factory=CallProcessor,
        ...
        with st.spinner("Evaluation in progress..."):
            with st.expander("Evaluation"):
                try:
                    ...
                    ax2.set_xticklabels(metric_cols, fontweight='bold')
